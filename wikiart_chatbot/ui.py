...
def create_ui(config: Optional[Config] = None) -> gr.Blocks:
    ...
    with gr.Blocks(
        title="WikiArt Chatbot", theme=gr.themes.Soft()
    ) as interface:
        gr.Markdown("# 👨‍🎨 WikiArt Chatbot")
        gr.Markdown(
            "Ask me anything about art! I can help you learn about famous "
            "artworks, artists, styles, and more."
        )
        ...
        def respond(
            message: str, chat_history: Optional[List[Dict]]
        ) -> Tuple[str, List[Dict]]:
