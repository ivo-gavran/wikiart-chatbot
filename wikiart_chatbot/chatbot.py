...
class WikiArtChatbot:
    """A chatbot that provides information about artworks.

    Uses semantic search and a language model to answer questions.
    """
    ...
    def load_data(self) -> None:
        ...
        # QUOTE_ALL mode properly handles commas in fields
        self.df = pd.read_csv(metadata_path, quoting=1)
        if index_path.exists():
            ...
            texts = self.df.apply(
                lambda row: (
                    f"{row['title']} by {row['artist']} - {row['style']}"
                    f" ({row['year']}): {row['description']}"
                ),
                axis=1,
            )
            embeddings = self.embedding_model.encode(
                texts.tolist(), show_progress_bar=True
            )
    ...
    def search_wikiart(
        ...
        Args:
            ...
            top_k: Optional number of results to return. Defaults to
                the config value.
        Returns:
    ...
    def query_ollama(self, context: str, user_input: str) -> str:
        ...
        try:
            ...
            raise OllamaError(
                "Could not connect to the AI model. Please ensure "
                "Ollama is running."
            )
    ...
    def _create_prompt(self, context: str, user_input: str) -> str:
        ...
        """
        return (
            f"You are an art expert assistant. Use the following context to "
            f"provide a detailed and engaging answer to the user's question.\n"
            "Be specific about the artworks mentioned and their historical "
            "significance.\n\n"
            "Context:\n"
            f"{context}\n\n"
            "Question:\n"
    ...
    def _process_ollama_response(self, response: requests.Response) -> str:
        ...
        if response.status_code != 200:
            ...
            raise OllamaError(
                "Received error from AI model (Status code: "
        ...
        if "response" not in response_data:
            ...
            raise OllamaError(
                "Received unexpected response format from AI model"
            )
...
Description: {row['description']}"""
    ...
    def process_message(
        ...
        try:
            ...
            if matches.empty:
                response = (
                    "I couldn't find any relevant artworks to answer your "
                    "question."
                )
            ...
            context = "\n\n".join(
                self.format_artwork_info(row) for _, row in matches.iterrows()
            )
